...
import {
  ...
  import { generatePinkishCaptchaImage } from '../../utils/captchaUtils';
  import {
  ...
  export async function handleStartVerification(interaction: ButtonInteraction) {
    ...
    // 1) Tạo captcha
    const { buffer, text } = generatePinkishCaptchaImage(5);
  ...
  `)
      ...
      collector.on('collect', async (msg: Message) => {
        ...
        if (code === correct) {
          ...
          await handleUserVerificationSuccess(member);
        } else {
          // Lấy member để xử lý thất bại
          const member = await guild.members.fetch(user.id);
          await handleUserVerificationFail(member);
        }
      });
