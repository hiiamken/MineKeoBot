...
export const logCommand = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName('user').setDescription('Người dùng cần kiểm tra lịch sử vi phạm').setRequired(true),
    ),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const infractions = await getInfractions(user.id, interaction.guild!.id);
