...
exports.getInfractions = getInfractions;
exports.getUserWarnings = getUserWarnings;
exports.clearInfractions = clearInfractions;
exports.clearUserWarnings = clearUserWarnings;
...
async function initDatabase() {
  ...
  `);
    ...
    if (!bankColumnExists) {
        await dbInstance.exec(`ALTER TABLE economy ADD COLUMN bank INTEGER NOT NULL DEFAULT 0;`);
    }
 ...
 */
async function logInfraction(guildId, userId, moderatorId, action, reason = "Không có lý do", duration = "", fine = 0) {
    const db = await initDatabase();
    await db.run(`INSERT INTO infractions (guild_id, user_id, moderator_id, action, reason, duration, fine) VALUES (?, ?, ?, ?, ?, ?, ?)`, [guildId, userId, moderatorId, action, reason, duration, fine]);
}
/**
 * Hàm ghi nhận cảnh cáo (warn)
 */
async function logWarning(guildId, userId, moderatorId, reason = "Không có lý do", fine = 0) {
    const db = await initDatabase();
    await db.run(`INSERT INTO warns (guild_id, user_id, moderator_id, reason, fine) VALUES (?, ?, ?, ?, ?)`, [guildId, userId, moderatorId, reason, fine]);
}
/**
...
async function getInfractions(guildId, userId) {
    const db = await initDatabase();
    return db.all(`SELECT * FROM infractions WHERE guild_id = ? AND user_id = ? ORDER BY timestamp DESC LIMIT 10`, [guildId, userId]);
}
/**
...
async function getUserWarnings(guildId, userId) {
    const db = await initDatabase();
    return db.all(`SELECT * FROM warns WHERE guild_id = ? AND user_id = ? ORDER BY timestamp DESC LIMIT 10`, [guildId, userId]);
}
/**
 * Xóa toàn bộ log vi phạm của một người dùng
 */
async function clearInfractions(guildId, userId) {
    const db = await initDatabase();
 ...
 */
async function getAllInfractions() {
    const db = await initDatabase();
