...
exports.handleConfirmLegitButton = handleConfirmLegitButton;
const discord_js_1 = require("discord.js");
const warnSystem_1 = require("../database/warnSystem");
async function handleConfirmLegitButton(interaction) {
    const { customId, user, message } = interaction;
    // Kiểm tra định dạng customId
    if (!customId.startsWith('confirm_legit_link_'))
        return;
    ...
    const linkContent = embed?.fields?.find((f) => f.name === '📎 Nội dung bạn đã gửi:')?.value;
    if (!linkContent) {
        return interaction.reply({
    ...
    await (0, warnSystem_1.confirmLegitLink)(userId, cleanLink);
    ...
    const confirmedEmbed = new discord_js_1.EmbedBuilder()
        .setColor('#DEA2DD')
        .setDescription(`✅ **Liên kết đã được xác nhận thành công!**\n` +
