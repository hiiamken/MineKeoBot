...
const economy_1 = require("../../database/economy"); // Import các hàm set tiền
exports.dataCommand = {
    data: new discord_js_1.SlashCommandBuilder()
        .setName('data')
        .setDescription('Quản lý dữ liệu người dùng (admin only)')
        .setDefaultMemberPermissions(discord_js_1.PermissionFlagsBits.Administrator) // Chỉ admin mới dùng được
        .addSubcommand((subcommand) => subcommand
        .setName('reset')
        .setDescription('Reset dữ liệu của một người dùng.')
        .addStringOption((option) => option.setName('target').setDescription('ID hoặc tên người dùng cần reset').setRequired(true))
        .addStringOption((option) => option.setName('data_type')
        .setDescription('Loại dữ liệu muốn reset.')
        .setRequired(true)
        .addChoices({ name: 'Levels', value: 'levels' }, { name: 'Money', value: 'money' }, { name: 'Bank', value: 'bank' }, { name: 'Messages', value: 'messages' }, { name: 'Invites', value: 'invites' }, { name: 'Voicetimes', value: 'voicetimes' }, { name: 'All', value: 'all' })))
        .addSubcommand((subcommand) => subcommand
        .setName('set')
        .setDescription('Set một giá trị cho dữ liệu của người dùng.')
        .addStringOption((option) => option.setName('target').setDescription('ID hoặc tên người dùng cần set').setRequired(true))
        .addStringOption((option) => option.setName('data_type')
        .setDescription('Loại dữ liệu muốn set.')
        .setRequired(true)
        .addChoices({ name: 'Levels', value: 'levels' }, { name: 'Money', value: 'money' }, { name: 'Bank', value: 'bank' }, { name: 'Messages', value: 'messages' }, { name: 'Invites', value: 'invites' }, { name: 'Voicetimes', value: 'voicetimes' }))
        .addIntegerOption((option) => option.setName('value').setDescription('Giá trị muốn set.').setRequired(true))),
    async execute(interaction) {
        ...
        if (subcommand === 'reset') {
            if (dataType === 'all') {
                ...
                switch (dataType) {
                    case 'levels':
                        updateColumn = 'xp = 0, level = 1';
                        break;
                    case 'money':
                        updateColumn = 'money = 0';
                        break;
                    case 'bank':
                        updateColumn = 'bank = 0';
                        break;
                    case 'messages':
                        updateColumn = 'message_count = 0';
                        break;
                    case 'invites':
                        updateColumn = 'invite_count = 0';
                        break;
                    case 'voicetimes':
                        updateColumn = 'total_time = 0';
                        break;
            ...
            switch (dataType) {
                ...
                case 'messages':
                    ...
                    break;
                case 'invites':
                    updateColumn = 'invite_count';
                    break;
                case 'voicetimes':
                    updateColumn = 'total_time';
                    break;
                default: return interaction.reply({ content: '❌ Loại dữ liệu không hợp lệ.', ephemeral: true });
            }
            if (dataType === 'money') {
                await (0, economy_1.adjustBalance)(interaction.guildId, targetUser.id, valueToSet - (await (await db.get(`SELECT money FROM economy WHERE guild_id = ? AND user_id = ?`, [interaction.guildId, targetUser.id]))?.money || 0));
            }
            else if (dataType === 'bank') {
                await (0, economy_1.adjustBankBalance)(interaction.guildId, targetUser.id, valueToSet - (await (await db.get(`SELECT bank FROM economy WHERE guild_id = ? AND user_id = ?`, [interaction.guildId, targetUser.id]))?.bank || 0));
            }
            else {
