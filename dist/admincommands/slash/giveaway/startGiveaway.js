...
exports.startGiveawayCommand = void 0;
 ...
 */
function parseBonusString(bonusStr) {
    if (!bonusStr.trim())
        return [];
...
exports.startGiveawayCommand = {
    ...
    async execute(interaction) {
        ...
        // Yêu cầu
        const hasRequire = requireRoleId !== '' ||
            requireLevel > 0 ||
            requireMoney > 0 ||
            requireInvite > 0;
        if (hasRequire) {
        ...
        const messageId = String(giveawayMessage.id);
        // 9) Thêm footer => "Message ID: ..."
        embed = discord_js_1.EmbedBuilder.from(embed)
            .setFooter({ text: `ID: ${messageId}` });
        await giveawayMessage.edit({ embeds: [embed] });
        // 10) Lưu DB
        // startGiveawayNumber(...) => THỨ TỰ THAM SỐ PHẢI KHỚP
        await (0, giveaway_1.startGiveawayNumber)(messageId, // ✅ Đảm bảo gửi đúng ID nhận từ Discord
        interaction.guild.id, channel.id, prize, winners, endTime, interaction.user.id, requireRoleId, requireLevel, requireMoney, requireInvite, bonusJSON);
        // 11) Reply ephemeral
